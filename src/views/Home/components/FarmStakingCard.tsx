...
const StyledFarmStakingCard = styled(Card)`
  background-image: url('/images/gft/2a.png');
...
const FarmedStakingCard = () => {
  ...
  const GFTPrice = usePriceCakeBusd().toNumber()
  ...
  return (
    <StyledFarmStakingCard>
      <CardBody>
        ...
        <Block>
          <Label>{TranslateString(546, 'GFT in Wallet')}</Label>
          <CakeWalletBalance cakeBalance={cakeBalance} />
          <Label>~${(GFTPrice * cakeBalance).toFixed(2)}</Label>
        </Block>
        <Actions>
          {account ? (
            <Button
            ...
            >
              {pendingTx
                ? TranslateString(548, 'Collecting GFT')
